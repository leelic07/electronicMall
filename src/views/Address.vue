      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="item,index in addressListLimit" :class="{'check':checkedItem === index}"
                    @click="checkedItem = index;selectedAddressId = item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" :class="{'open':isShowMore}" @click="showMore()">
                more
                <i class="i-up-down">
          ...
          <div class="next-btn-wrap">
            ...
            <router-link class="btn btn--m btn--red"
                         :to="{path:'/orderConfirm',query:{addressId:selectedAddressId}}">Next
            </router-link>
          </div>
        </div>
      </div>
        ...
        axios.get('/mall/users/addressList').then(response => {
          ...
          if (res.status === '0') {
            ...
            this.addressList.forEach((item, index) => {
              item.isDefault === true && (this.checkedItem = index) && (this.selectedAddressId = item.addressId);
            });
          }
...
//          else this.$message.error(res.msg);
      ...
      setDefault(addressId){
        axios.post('/mall/users/setDefault', {
          addressId: addressId
        }).then(response => {
          let res = response.data;
          if (res.status === '0') {
      ...
      delAddress(addressId){
        this.$confirm('确定删除地址？', '提示', {
          ...
          type: 'warning'
            ...
            axios.post('/mall/users/delAddress', {
              addressId: addressId
            }).then(response => {
              let res = response.data;
              if (res.status === '0') {
